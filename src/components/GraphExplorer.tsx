...
export function GraphExplorer({
  ...
  const neighbours = useMemo(() => {
    ...
    for (const l of links) {
      const s = typeof l.source === "string" ? l.source : (l.source as FGNode).id;
      const t = typeof l.target === "string" ? l.target : (l.target as FGNode).id;
      if (s === selectedId && byId.get(t)) out.push({ node: byId.get(t)!, rel: l.rel });
      else if (t === selectedId && byId.get(s)) out.push({ node: byId.get(s)!, rel: l.rel });
    }
    return out;
        ...
        {!selected ? (
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="font-display text-lg leading-tight">{selected.label}</div>
            </div>
                ...
                {neighbours.map((nb, i) => (
                  <li key={i}>
                    ...
                    >
                      ...
                      />
                      <span className="text-[var(--color-muted)]">{nb.rel.toLowerCase()}</span>{" "}
                      {nb.node.label}
                    </button>
                  </li>
                ))}
